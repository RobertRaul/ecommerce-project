    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex items-center justify-between">
                <Link href="/admin/categorias" className="inline-flex items-center space-x-2 text-gray-600 hover:text-gray-900 transition">
                ...
                <button type="submit" disabled={loading} className="inline-flex items-center space-x-2 bg-purple-600 text-white px-6 py-3 rounded-lg font-semibold hover:bg-purple-700 transition disabled:opacity-50 disabled:cursor-not-allowed shadow-lg hover:shadow-xl">
                    ...
                    <span>{loading ? 'Guardando...' : isEdit ? 'Actualizar Categoría' : 'Crear Categoría'}</span>
                </button>
            </div>
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                ...
                <div className="space-y-6">
                    <div>
                        ...
                        <input type="text" name="name" value={formData.name} onChange={handleChange} className={`w-full px-4 py-2.5 border rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition ${errors.name ? 'border-red-500 bg-red-50' : 'border-gray-300'}`} placeholder="Ej: Electrónica" />
                        {errors.name && (
                    ...
                    <div>
                        ...
                        <textarea name="description" value={formData.description} onChange={handleChange} rows={4} className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 resize-none" placeholder="Describe la categoría..." />
                    </div>

                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-2">Orden de visualización</label>
                        <input type="number" name="order" value={formData.order} onChange={handleChange} min="0" className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" placeholder="0" />
                ...
                <div className="space-y-4">
                    ...
                    {displayImage && (
                        <div className="relative group">
                            ...
                            <button type="button" onClick={imagePreview ? removeImage : removeExistingImage} className="absolute top-2 right-2 bg-red-600 text-white p-2 rounded-full opacity-0 group-hover:opacity-100 transition shadow-lg hover:bg-red-700">
                                <X className="h-5 w-5" />
                            </button>
                        </div>
                    )}
            ...
            <div className="bg-white rounded-lg shadow-sm p-6 border border-gray-200">
                ...
                <label className="flex items-center space-x-3 cursor-pointer p-4 border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                    ...
                    <div>
                        <span className="text-sm font-medium text-gray-900 block">Categoría Activa</span>
            ...
            <div className="flex items-center justify-between pt-6 border-t border-gray-200">
                <Link href="/admin/categorias" className="text-gray-600 hover:text-gray-900 font-medium">Cancelar</Link>
